...
/*
 * Aviso: Esta clase ha sido modificada.
 */
...
class Jumble {
     ...
     * @return string
     */
    function decrypt ($source, $key = null) {
        $this->errors = array();

        if($key == null) {
            $key = substr($source, 0, 8);
     ...
     * @return string
     */
    function encrypt($source, $key = null, $sourcelen = 0) {
        $this->errors = array();
...
//var_dump($adj);
     ...
     * @return string
     */
    private function _genKey($length = 8) {
...
$s = $jumble->encrypt('qwerty', $key);

echo $s;
echo "\n";
echo $jumble->decrypt($s);
echo "\n";
//echo "\n";
